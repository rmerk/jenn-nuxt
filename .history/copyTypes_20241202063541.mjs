...
if (!fs.existsSync(normalizedDestinationPath)) {
  fs.writeFileSync(normalizedDestinationPath, { recursive: true })
}
...
const modifiedNormalizedContent = normalizedContent.replace('@strapi/strapi', '@strapi/types')
...
fs.writeFile(normalizedDestinationPath, modifiedContent, (err) => {
  if (err) {
    console.error(`Error writing to destination file: ${err}`)
    process.exit(1)
  }
