...
const { data: products } = await useAsyncData('products', () => find('products', {
    populate: '*',
}));
</script>
...
<template>
    <div class="bg-white py-12">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mt-12">
                <div v-for="product in products" :key="product.id" class="mb-12">
                    <h2 class="text-2xl font-bold text-amber-700 mb-6">
                        {{ product.name }}
                    </h2>
                    <div class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                        <div v-for="item in product.items" :key="product.id" class="bg-white rounded-lg shadow-md overflow-hidden">
