...
const destinationDir = path.join(__dirname, `../jenn-site/${destinationFolder}`);

const files = [
  {
    src: path.join(__dirname, "../dough-boy/types/generated/contentTypes.d.ts"),
    dest: path.join(__dirname, `${destinationDir}/${destinationFolder}/contentTypes.d.ts`),
  },
  {
    src: path.join(__dirname, "../dough-boy/types/generated/components.d.ts"),
    dest: path.join(__dirname, `${destinationDir}/${destinationFolder}/components.d.ts`),
  },
];

function copyFile({ src, dest }: { src: string; dest: string }) {
